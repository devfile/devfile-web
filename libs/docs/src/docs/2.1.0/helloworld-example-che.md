...
1. For the hello world example, we will create a simple [hello world expressjs](https://expressjs.com/en/starter/hello-world.html) application

2. In Eclipse Che, create an empty workspace

3. Close and stop the workspace

4. Go to 'Edit the workspace' and view the devfile. The devfile should be populated with content similar to the following
...
5. Change the value following component fields to set the component name to `runtime`
...
6. Change the `sourceMapping` to `/projects/helloworld-example` in the `runtime` component to set the project directory

7. The `runtime` container will host the expressjs app which listens on port `3000`, define this port in the component by specifying an entry under 
...
11. Before we can run the devfile, create the files which make up the simple [hello world expressjs](https://expressjs.com/en/starter/hello-world.html) application
...
12. Run the `install` command by opening the menu, open 'Terminal/Run Task', under the 'Run Task' menu open 'devfile/devfile: install', the task should open
...
14. Under the 'EXLPORER', expand the 'ENDPOINTS' panel and copy the `http-3000` endpoint URL
